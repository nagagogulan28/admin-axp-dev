...
class VendorAdjustmentResp extends Model
{
    ...
    public function get_vendor_adjustments($fromdate,$todate){
        ...
        $where_condition = "DATE_FORMAT($this->table.created_date,'%Y-%m-%d')>=:fromdate AND DATE_FORMAT($this->table.created_date,'%Y-%m-%d')<=:todate AND appxpay_adjustment_status='N'";
