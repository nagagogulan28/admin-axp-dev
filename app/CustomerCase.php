...
class CustomerCase extends Model
{
    ...
    public function get_all_cases(){
        ...
        $query = "SELECT id,case_gid,transaction_gid,transaction_amount,customer_name,customer_reason,`status`,DATE_FORMAT(created_date,'%d-%m-%Y %h:%i:%s %p') as created_date,appxpay_caseid FROM $this->table
    ...
    public function get_custcase_appxpay($id){

        $where_condition = "appxpay_caseid=:appxpay_caseid";
        $apply_condition["appxpay_caseid"] = $id;
