...
class AppOption extends Model
{
   ...
   public static function get_category_post_count(){
     ...
     $query = "SELECT option_value AS blog_category,count(post_category) no_of_posts FROM app_option LEFT JOIN appxpay_blog ON appxpay_blog.post_category = app_option.id  WHERE module='blog_category' AND appxpay_blog.post_status='active' GROUP BY option_value";
   ...
   public static function get_appxpay_cdr()
   {    
        return DB::table("app_option")->select("id","option_value")->where("module","appxpay_cdr")->get();
   }
