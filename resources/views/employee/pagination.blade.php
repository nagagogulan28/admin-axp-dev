...
<table class="table table-bordered">
    ...
    <tbody>
        ...
        <tr>
            ...
            @if(auth()->guard('employee')->user()->user_type == '1')
            <td><a href="/appxpay/hrm/employee-accesses/{{$employee->id}}" class="btn btn-primary btn-sm">Access</a>
...
@if($module == "appxpay_adjustment")
@if(isset($appxpay_adjustments))
@php($table_count=0)
<div class="table-responsive">
    <table class="table table-bordered">
        <thead>
        ...
        <tbody id="appxpay_adjustmenttable">
            @if(count($appxpay_adjustments)>0)

            @foreach($appxpay_adjustments as $appxpay_adjustment)
            <tr>
                <td>
                    <div class="checkbox"><label><input type="checkbox" name="id[]" id="id" value="{{$appxpay_adjustment->id}}" class="form-control" onclick="getBulkAdjsutmentIds(this)" {{($appxpay_adjustment->adjustment_status=="processed")?'disabled':''}}> <span class="cr"><i class="cr-icon fa fa-check"></i></span>
                        </label></div>
                </td>
                <td>{{++$table_count}}</td>
                <td>{{$appxpay_adjustment->merchant_traxn_id}}</a> </td>
                <td>{{number_format($appxpay_adjustment->traxn_amount,$decimal_length)}}</td>
                <td>{{$appxpay_adjustment->bankname}}</td>
                <td>{{$appxpay_adjustment->merchant_traxn_method}}</td>
                <td>{{$appxpay_adjustment->adjustment_charges_per}}%</td>
                <td>{{$appxpay_adjustment->adjustment_charges}}</a> </td>
                <td>{{$appxpay_adjustment->adjustment_gst_per}}%</td>
                <td>{{$appxpay_adjustment->adjustment_gst}}</td>
                <td>{{$appxpay_adjustment->total_charge}}</td>
                <td>{{$appxpay_adjustment->adjustment_amount}}</td>
                <td>{{$appxpay_adjustment->adjustment_status}}</td>
                <td>{{$appxpay_adjustment->created_date}}</td>
                <td><a href="javascript:void(0)" onclick="merchantAdjustment('{{$appxpay_adjustment->id}}','{{$appxpay_adjustment->merchant_id}}')" class="btn btn-success btn-sm text-center">Proceed <br>to<br> merchant</button></td>
            </tr>
            @endforeach
            @else
            <tr>
    ...
    <div class="col-sm-6">
        <h5 class="pagination">Showing {{$appxpay_adjustments->firstItem()==''?'0':$appxpay_adjustments->firstItem()}} to {{$appxpay_adjustments->lastItem() =='' ? '0':$appxpay_adjustments->lastItem()}} of {{$appxpay_adjustments->total()}} entries</h5>
    </div>
    <div class="col-sm-6 text-right">
        <ul class="pagination">
            <li><a href="{{$appxpay_adjustments->previousPageUrl()}}">Previous</a></li>
            <li><a href="javascript:">{{$appxpay_adjustments->currentPage()}}</a></li>
            <li><a href="{{$appxpay_adjustments->nextPageUrl()}}">Next</a></li>
        </ul>
    </div>
</div>
@endif
    ...
    <table class="table table-bordered">
        ...
        <tbody id="bginfotable">
            ...
            <tr>
                ...
                @else
                <td class="btn-same"><a href="/risk-complaince/merchant-document/view/{{$document->merchant_id}}" class="btn btn-success btn-sm">Success</a></td>
                @endif
            </tr>
            @endforeach
            @else
...
<div class="table-responsive">
    <table class="table table-bordered">
        ...
        <tbody id="customertable">
            ...
            <tr>
                <td>{{++$table_count}}</td>
                <td><a href="{{route('get-case-details',$custcase->appxpay_caseid)}}">{{$custcase->case_gid}}</a></td>
    ...
    <table class="table table-bordered">
        ...
        <tbody id="customertable">
            ...
            <tr>
                ...
                <td>{{appxpayaSupOrderInv::suporder_option($suppaybatch->batch_invno)}}</td>
                @elseif($suppaybatch->batch_invtype == '2')
                <td>{{appxpaySupExpInv::supexp_option($suppaybatch->batch_invno)}}</td>
                @else
                <td>{{appxpaySupCDNote::supplier_option($suppaybatch->batch_invno)}}</td>
                @endif
    ...
    <table class="table table-bordered">
        ...
        <tbody id="customertable">
            ...
            <tr>
                ...
                <td>{{appxpayCustOrderInv::custorder_option($custrcptentry->receipt_invno)}}</td>
                @else
                <td>{{appxpayaCustCDNote::custnote_option($custrcptentry->receipt_invno)}}</td>
                @endif
...
@if($module == "vendoradjustment")
    @if(isset($vendoradjustments))
        ...
        <div class="table-responsive" id="">
            <form id="appxpay-adjustment-form" method="POST">
                <table class="table table-bordered">
                    <thead>
                        <tr>
                            <th>Sno</th>
...
@if($module == "appxpayadjustment")
@if(isset($appxpayadjustments))
@php($table_count=0)
<div class="table-responsive" id="">
    <table class="table table-bordered">
        <thead>
        ...
        <tbody id="bginfotable">
            @if(count($appxpayadjustments)>0)
            @foreach($appxpayadjustments as $appxpayadjustment)
            <tr>
                <td>{{++$table_count}}</td>
                <td>{{$appxpayadjustment->merchant_gid}}</td>
                <td>{{$appxpayadjustment->merchant_transaction_id}}</td>
                <td>{{number_format($appxpayadjustment->transaction_amount,2)}}</td>
                <td>{{$appxpayadjustment->charges_per}}%</td>
                <td>{{$appxpayadjustment->charges_on_transaction}}</td>
                <td>{{$appxpayadjustment->gst_per}}%</td>
                <td>{{$appxpayadjustment->gst_on_transaction}}</td>
                <td>{{$appxpayadjustment->total_amt_charged}}</td>
                <td>{{$appxpayadjustment->adjustment_amount}}</td>
                <td>{{$appxpayadjustment->created_date}}</td>
            </tr>
            @endforeach
            @else
            <tr>
    ...
    <div class="col-sm-6">
        <h5 class="pagination">Showing {{$appxpayadjustments->firstItem()==''?'0':$appxpayadjustments->firstItem()}} to {{$appxpayadjustments->lastItem() =='' ? '0':$appxpayadjustments->lastItem()}} of {{$appxpayadjustments->total()}} entries</h5>
    </div>
    <div class="col-sm-6 text-right">
        <ul class="pagination">
            <li><a href="{{$appxpayadjustments->previousPageUrl()}}">Previous</a></li>
            <li><a href="javascript:">{{$appxpayadjustments->currentPage()}}</a></li>
            <li><a href="{{$appxpayadjustments->nextPageUrl()}}">Next</a></li>
        </ul>
    </div>
</div>
@endif
...
@if($module == "appxpayroute")
@if(isset($appxpayroutes))
@php($table_count=0)
<div class="table-responsive" id="">
    <table class="table table-bordered">
        <thead>
            <tr>
                ...
                <th>Business Name</th>
                <th>appxpay Api</th>
                <th>appxpay Security Key</th>
                <th>Created Date</th>
            </tr>
        </thead>
        <tbody id="bginfotable">
            @if(count($appxpayroutes)>0)
            @foreach($appxpayroutes as $appxpayroute)
            <tr>
                <td>{{++$table_count}}</td>
                <td><a href="javascript:void(0)" onclick="editappxpayKey('{{$appxpayroute->id}}')">{{$appxpayroute->business_name}}</a></td>
                <td>{{$appxpayroute->app_id}}</td>
                <td>{{$appxpayroute->secret_key}}</td>
                <td>{{$appxpayroute->created_date}}</td>
            </tr>
            @endforeach
            @else
            <tr>
    ...
    <div class="col-sm-6">
        <h5 class="pagination">Showing {{$appxpayroutes->firstItem()==''?'0':$appxpayroutes->firstItem()}} to {{$appxpayroutes->lastItem() =='' ? '0':$appxpayroutes->lastItem()}} of {{$appxpayroutes->total()}} entries</h5>
    </div>
    <div class="col-sm-6 text-right">
        <ul class="pagination">
            <li><a href="{{$appxpayroutes->previousPageUrl()}}">Previous</a></li>
            <li><a href="javascript:">{{$appxpayroutes->currentPage()}}</a></li>
            <li><a href="{{$appxpayroutes->nextPageUrl()}}">Next</a></li>
        </ul>
    </div>
</div>
@endif
